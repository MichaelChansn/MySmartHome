   ...
   //判断网络连接的类型
	...
	private BroadcastReceiver connectionReceiver = new BroadcastReceiver() { 
		...
		public void onReceive(Context context, Intent intent) { 
			...
			OPT_ActivityView_static.sendHandlerMessage(Main_Control.handler, "检测到网络状态发生变化。。。", OPT_ActivityView_static.INDEX_TEXTVIEW);
		...
		// 没有任何网络可用
			...
			OPT_ActivityView_static.sendHandlerMessage(Main_Control.handler, "手机的所有网络都没连接。。。", OPT_ActivityView_static.INDEX_TEXTVIEW);
		}
		else 
		   if(mobNetInfo.isConnected() && !wifiNetInfo.isConnected())
			{ 
			// 手机网络可用
			    ...
			    OPT_ActivityView_static.sendHandlerMessage(Main_Control.handler, "手机3G信号开启。。。", OPT_ActivityView_static.INDEX_TEXTVIEW);
			} 
			else
				if(!mobNetInfo.isConnected() && wifiNetInfo.isConnected())
				{
					//wifi可用
					   ...
					   OPT_ActivityView_static.sendHandlerMessage(Main_Control.handler, "WIFI信号开启。。。", OPT_ActivityView_static.INDEX_TEXTVIEW);
				...
				else
				{
				 //3g和wifi都可用
					...
					OPT_ActivityView_static.sendHandlerMessage(Main_Control.handler, "WIFI和3G信号都开启。。。", OPT_ActivityView_static.INDEX_TEXTVIEW);
			    }
		} 
		};
